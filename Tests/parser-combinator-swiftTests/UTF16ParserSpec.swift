...
class UTF16ParserSpec: QuickSpec {
    private typealias P = ParserCombinatorSwift.UTF16Parser
    ...
    override func spec() {
        ...
        it("string") {
            let input = "abc"
            let output = try! P.string(input).parse(input.utf16, input.startIndex).unwrap()
            expect(input).to(equal(output))
        }
        it("stringIn") {
            let input = "\u{2000}\u{2002}"
            let parser = P.stringIn("\u{2000}", "\u{2002}").rep(1).map { $0.joined(separator: "") }
            let output = try! parser.parse(input.utf16, input.startIndex).unwrap()
            expect(input).to(equal(output))
        }
    }
}
